    ...
    <div className="space-y-4">
      {!imagePreview ? (
        ...
        >
          <input {...getInputProps()} />
          <div>
            <UploadAnimated width={48} height={48} stroke="#9ca3af" className="mb-4" />
          </div>
        ...
        <div className="relative rounded-lg overflow-hidden bg-gray-100 h-64 w-64 mx-auto">
          ...
          <div className="absolute top-2 right-2">
            <Button variant="secondary" size="sm" onClick={removeFile} className="bg-white/80 hover:bg-white">
              <X className="h-4 w-4" />
            </Button>
          </div>
          <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/60 to-transparent">
            <p className="text-sm font-medium text-white truncate">{currentImage?.name}</p>
            <p className="text-xs text-white/80">
              {currentImage ? (currentImage.size / 1024 / 1024).toFixed(2) : 0} MB
            </p>
          </div>
        </div>
      )}
